...
import {
  Search, Filter, Eye, Phone, MapPin, Truck, Store, Package, X, Loader2, Building2, Scale, FileDown, Edit, Save, Plus
} from 'lucide-react';
...
import { useOrders, useAllOrders, useUpdateOrderStatus, useBulkUpdateOrderStatus, useUpdateOrder, OrderWithItems, OrderStatus } from '@/hooks/useOrders';
...
const EmployeeDashboard: React.FC = () => {
  ...
  const handleBulkStatusChange = async (newStatus: OrderStatus) => {
    try {
      ...
      toast({ title: 'Error updating orders', description: 'You may not have permission to perform this action.', variant: 'destructive' });
    }
  };

  const handleSingleStatusChange = async (orderId: string, newStatus: OrderStatus) => {
    try {
      await updateStatus.mutateAsync({ id: orderId, status: newStatus });
      toast({ title: 'Updated', description: `Order status changed to ${statusLabels[newStatus]}.` });
    } catch (error) {
      toast({ title: 'Error updating status', description: 'You may not have permission to perform this action.', variant: 'destructive' });
    }
  };
        ...
        <Card className="border-primary"><CardContent className="p-4">
          <div className="flex items-center justify-between flex-wrap gap-4">
            ...
            <div className="flex items-center gap-2">
              <Select value={bulkAction} onValueChange={setBulkAction}><SelectTrigger className="w-full sm:w-[180px]"><SelectValue placeholder="Change status..." /></SelectTrigger><SelectContent>{Object.keys(statusLabels).map(s => <SelectItem key={s} value={s}>{statusLabels[s as OrderStatus]}</SelectItem>)}</SelectContent></Select>
