...
import {
    Search, Filter, Phone, MapPin, Truck, Store, Package, X, Loader2, Building2, Scale, FileDown, Edit, Plus
} from 'lucide-react';
import { format, parseISO } from 'date-fns';
import { DateRangePicker } from '@/components/ui/date-range-picker';
import { DateRange } from 'react-day-picker';
import { useToast } from '@/hooks/use-toast';
import { useOrders, useAllOrders, useUpdateOrderStatus, useBulkUpdateOrderStatus, OrderWithItems, OrderStatus } from '@/hooks/useOrders';
...
const EmployeeOrders: React.FC = () => {
    ...
    const hasActiveFilters = useMemo(() => {
        ...
        delete (filtersWithoutSearch as any).searchQuery;
    ...
    return (
        <div className="space-y-6">
            <div>
                <h1 className="text-2xl font-bold">Orders (Employee)</h1>
                    ...
                    <>
                        ...
                        <div className="space-y-6">
                            ...
                            <div><h4 className="font-semibold mb-3">Order Items</h4><div className="space-y-2">{selectedOrder.items.map((item, index) => (<div key={index} className="flex justify-between items-center p-3 bg-muted/50 rounded-lg">                      <div>
                                <p className="font-medium">{(item.product_name_ar || item.product_name_fr || 'No Product Name Available')}</p>
                                {item.weight && (
